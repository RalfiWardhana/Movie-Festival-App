...
func StatsRoutes(r *gin.RouterGroup, StatsHandler *handler.StatsHandler) {
	stats := r.Group("/stats")
	{
		...
		stats.POST("/:movie_id/vote", middleware.AuthMiddleware(), middleware.RoleMiddleware("user"), StatsHandler.VoteMovie)                   // Authenticated users can vote
		stats.POST("/:movie_id/unvote", middleware.AuthMiddleware(), middleware.RoleMiddleware("user"), StatsHandler.UnvoteMovie)               // Authenticated users can unvote
		...
		stats.POST("/:movie_id/trace", middleware.AuthMiddleware(), middleware.RoleMiddleware("user"), StatsHandler.TraceViewership)
